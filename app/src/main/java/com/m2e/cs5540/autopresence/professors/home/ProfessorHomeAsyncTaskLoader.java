...
public class ProfessorHomeAsyncTaskLoader
   ...
   @Override public AsyncLoaderStatus loadInBackground() {
      ...
      try {
         List<Course> coursesList =
               DatabaseUtil.getInstance().getCoursesByProfId(
                     AppContext.getCurrentAppContext().getUser().getId());
         Log.i(TAG,
               "Professor course registered size is: " + coursesList.size());
         if (coursesList != null) {
            ...
            for (int i = 0; i < coursesList.size(); i++) {
               ...
               courseAttendancePercent.setAttendancePercent(
                     String.format("%.2f %%",
                           calculateAttendancePercent(course)));
   ...
   private double calculateAttendancePercent(Course course) {
      ...
      List<UserAttendance> userAttendanceList =
            DatabaseUtil.getInstance().getCourseAttendances(course.getId());
      if (courseEnrollmentList != null && courseEnrollmentList.size() > 0 &&
            userAttendanceList != null && userAttendanceList.size() > 0) {
